...
func (s *CallbackResponseSchema) encodeFields(e *jx.Encoder) {
	{
...
var jsonFieldsNameOfCallbackResponseSchema = [3]string{
	0: "id_token",
	1: "email",
	2: "password",
}
...
func (s *CallbackResponseSchema) Decode(d *jx.Decoder) error {
	if s == nil {
		return errors.New("invalid: unable to decode CallbackResponseSchema to nil")
	}
	...
	if err := d.ObjBytes(func(d *jx.Decoder, k []byte) error {
		...
		return errors.Wrap(err, "decode CallbackResponseSchema")
	}
	...
	for i, mask := range [1]uint8{
		...
		if result := (requiredBitSet[i] & mask) ^ mask; result != 0 {
			...
			for bitN := 0; bitN < missed; bitN++ {
				...
				fieldIdx := i*8 + bitIdx
				var name string
				if fieldIdx < len(jsonFieldsNameOfCallbackResponseSchema) {
					name = jsonFieldsNameOfCallbackResponseSchema[fieldIdx]
				} else {
